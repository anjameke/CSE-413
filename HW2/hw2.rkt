...
(define (eval-tree expr-tree)
    ...
    (let ((op (operator expr-tree))
          ...
          (expr2 (if (number? (right-op expr-tree)) (right-op expr-tree)
...
(define (map-leaves f expr-tree)
    (cond ((null? expr-tree) '())
          ((and (number? (left-op expr-tree)) (number? (right-op expr-tree)))
           (flatten (append (list (f (left-op expr-tree))
                                  (operator expr-tree)
                                  (f (right-op expr-tree))))))
          ((and (number? (left-op expr-tree)) (not (number? (right-op expr-tree))))
           (flatten (append (list (f (left-op expr-tree))
                                  (operator expr-tree)
                                  (map-leaves f (right-op expr-tree))))))
          ((and (number? (right-op expr-tree)) (not (number? (left-op expr-tree))))
           (flatten (append (list (map-leaves f (left-op expr-tree))
                                  (operator expr-tree)
                                  (f (right-op expr-tree))))))
          ((and (not (number? (left-op expr-tree))) (not (number? (right-op expr-tree))))
           (flatten (append (list (map-leaves f (left-op expr-tree))
